...
pub type ResponseBuilder = MessageBuilder<u16>;
...
impl ResponseBuilder {
    ...
    pub fn build(self) -> Result<Response, InvalidStatusCode> {
        Ok(Response {
            info_line: ResponseLine::new(StatusCode::from_u16(
                self.info_line,
            )?),
...
impl TryFrom<OneResponse> for Response {
    type Error = InvalidStatusCode;

    fn try_from(mut res: OneResponse) -> Result<Response, InvalidStatusCode> {
...
mod tests {
    ...
    fn test_one_to_two_response_minimal() {
        ...
        let result = Response::try_from(one).unwrap();
    ...
    fn test_one_to_two_response_custom_header() {
        ...
        let verify =
            Response::builder().status(200).headers(headers).build().unwrap();
        assert_eq!(result, verify);
    }
