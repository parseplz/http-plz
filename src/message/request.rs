...
impl TryFrom<OneRequest> for Request {
    type Error = InvalidUri;

    fn try_from(mut req: OneRequest) -> Result<Self, InvalidUri> {
        let body = req.take_body();
        let (info_line, headers) = req.message_head.into_parts();
        let (headers, body) = process_one_headers_and_body(headers, body);
        let (raw_method, raw_path, _) = info_line.into_parts();
...
mod tests {
    ...
    fn test_one_to_two_request_minimal() {
        ...
        assert_eq!(Request::try_from(one).unwrap(), verify);
    }

    #[test]
    fn test_one_to_two_request_full_url() {
        let uri = Uri::try_from("http://www.google.com/robots.txt").unwrap();
        let verify = Request::builder().method(Method::GET).uri(uri).build();
        let input = "GET http://www.google.com/robots.txt HTTP/1.1\r\n\r\n";
        let one = OneRequest::try_from(BytesMut::from(input)).unwrap();
        assert_eq!(Request::try_from(one).unwrap(), verify);
    }
